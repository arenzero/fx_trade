...
def trade(current_day = '2021-06-14',posses = {'JPY':10000,'USD':0},df = pd.read_csv('C:/Users/masay/Documents/Python Scripts/FX_trade/fx_trade/temp_data/jpy_usd.csv')):
    ...
    df_c[df_c.columns[~df_c.columns.isin([col for col in df_c.columns if 'macd' in col])]] = (df_c[df_c.columns[~df_c.columns.isin([col for col in df_c.columns if 'macd' in col])]].div(df_c['Close'],axis=0)-1)*100
